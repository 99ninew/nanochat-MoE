...
class GPT(nn.Module):
    ...
    def estimate_flops(self, seq_len: int | None = None):
        """Estimate FLOPs per token for a given sequence length.

        Uses the same PaLM Appendix B-style approximation as `estimate_mfu`.
        If seq_len is None, falls back to the model's configured block size.
        """
        N = self.get_num_params()
        N_embedding = self.transformer.wte.weight.numel()
        cfg = self.config
        L = cfg.n_layer
        H = cfg.n_head
        Q = cfg.n_embd // cfg.n_head
        T = cfg.block_size if seq_len is None else seq_len
        flops_per_token = 6 * (N-N_embedding) + 12 * L * H * Q * T
